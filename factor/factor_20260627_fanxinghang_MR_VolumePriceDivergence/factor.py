...
def compute(gen, params):
    ...
    price_chg = gen.close.pct_change(win1)
    vol_chg = gen.volume.rolling(win1, min_periods=max(10, win1 // 2)).mean().pct_change(win1)
    raw = cs_zscore(gen, vol_chg) - cs_zscore(gen, price_chg)
