...
def compute(gen, params):
    ...
    win2 = int(params.get('win2', 60))
    win3 = int(params.get('win3', 5))
    high_N = gen.high.rolling(win1, min_periods=max(10, win1 // 2)).max()
    low_N = gen.low.rolling(win1, min_periods=max(10, win1 // 2)).min()
    range_pos = (gen.close - low_N) / (high_N - low_N + 1e-06)
    high_L = gen.high.rolling(win2, min_periods=max(30, win2 // 2)).max()
    low_L = gen.low.rolling(win2, min_periods=max(30, win2 // 2)).min()
    range_pos_L = (gen.close - low_L) / (high_L - low_L + 1e-06)
